...
namespace Dennokoworks.MeshModularizer
{
    ...
    [InitializeOnLoad]
    internal static class MeshModularizerVersion
    {
        // version.json の GUID (アセット移動に対応するため GUID 経由でパス解決する)
        private const string VersionJsonGuid = "";
        ...
        {
            var windows = Resources.FindObjectsOfTypeAll<MeshModularizerWindow>();
        ...
        {
            // 1) GUID 経由（アセット移動に追従。ただし AssetDatabase 準備前や GUID 未設定時は空を返す）
            if (!string.IsNullOrEmpty(VersionJsonGuid))
            {
        ...
        private static string TryReadVersion(string path)
        {
            ...
            {
                Debug.LogWarning($"[MeshModularizerVersion] Failed to read version.json ({path}): {e.Message}");
            }
            return null;
        }
